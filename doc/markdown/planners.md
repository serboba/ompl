...
- **Single-query planners**<br>
  ...
  - [Rapidly-exploring Random Trees (RRT)](\ref gRRT)<br>
    ...
    - [Low-Actions RRT (LARRT)](\ref gLARRT)<br>
      LARRT is a bidirectional RRT for factored state spaces (configurations partitioned into independent object groups) that minimizes the number of actions (mode-switches between groups) rather than path length. Multi-group motions are isolated into single-group steps and a path defragmenter reorders and merges same-group segments to reach the minimal action count. Intended for rearrangement and multi-object manipulation problems.
