  ...
  : si_(std::move(si)), fragment_indices(fragment_indices)
{
    ...
    opt_ = std::make_shared<base::MinimalActionsObjective>(si_, fragment_indices);
}

std::vector<int> ompl::geometric::PathDefragmenter::getChangedGroups(const std::vector<double> &from_,
...
void ompl::geometric::PathDefragmenter::isolateStates(const base::State *rfrom, const base::State *rto,
                                                      std::vector<ompl::base::State *> &iso_)
{
...
void ompl::geometric::PathDefragmenter::getIntermediateState(const ompl::base::State *from, const ompl::base::State *to,
                                                             ompl::base::State *state, int index_group)
{
...
void ompl::geometric::PathDefragmenter::reConnect(ompl::base::State *from,
                                                  ...
                                                  std::vector<ompl::base::State *> &rewireResult)
{
    if (reConnect(from, prio_, rewireResult))
    {
        ...
        if (reConnect(temp_from, stack_, rewireResult))
        {  // reconnection successful
            si_->freeState(temp_from);
        }
        else
        {  // collision during reconnection
            si_->freeState(temp_from);
            freeStates(rewireResult);
        }
    }
    else
    {  // collision during first reconnection
        freeStates(rewireResult);
    }
...
void ompl::geometric::PathDefragmenter::findNextFragment(
    int start_index, int prev_index, std::vector<ompl::base::State *> &mainPath, bool sameFragmentType,
...
int ompl::geometric::PathDefragmenter::pathDefrag(std::vector<ompl::base::State *> &mainPath)
{
    ...
    for (size_t toID = 1; toID < mainPath.size() - 1; ++toID)
    {
        ...
        if (getChangedIndex(fromState, toState) != prev_index)
        {
            ...
            {  // no rewiring possible because of invalid motion but no problem, just start with next the next state and
               // go on
                prev_index = getChangedIndex(mainPath.at(toID - 1), mainPath.at(toID));
                continue;
            }
...
void ompl::geometric::PathDefragmenter::getFragmentIDs(std::vector<ompl::base::State *> &path,
                                                       std::vector<FragmentIntervals> &fragmentIDs,
                                                       bool goalFragment = false)
{
    fragmentIDs.clear();
    ...
    for (size_t i = 1; i < path.size() - 1; i++)
    {
        if (prev_index != getChangedIndex(path.at(i), path.at(i + 1)))
        {
            ...
            {
                // equal states can appear when fragments are skipped; drop the duplicate
...
void ompl::geometric::PathDefragmenter::getFragment(int start_index, int end_index,
                                                    std::vector<ompl::base::State *> &mainPath,
...
void ompl::geometric::PathDefragmenter::skipFragments(std::vector<ompl::base::State *> &mainPath)
{
    ...
    for (size_t i = 0; i + 1 < fragmentIDs.size(); i++)
    {
        ...
        else
        {
            ...
            if (!reConnect(temp_from, pathB, result_))
            {  // reconnect only one path segment directly to the state
                freeStates(pathB);
                freeStates(result_);
                si_->freeState(temp_from);
                continue;
...
void ompl::geometric::PathDefragmenter::simplifyActionIntervals(
    std::vector<ompl::base::State *> &mainPath)  // finds intervals of same action index, builds shortcuts
{
...
void ompl::geometric::PathDefragmenter::checkRepairPath(std::vector<ompl::base::State *> &path_)
{
    for (size_t i = 0; i + 1 < path_.size();)
    {
        ...
        if (opt_->motionCost(path_.at(i), path_.at(i + 1)).value() > 1.0)  // path defrag needs every edge to have
                                                                           // motion cost 1.0
        {
...
ompl::geometric::PathDefragmenter::clonePath(const std::vector<ompl::base::State *> &path)
{
